...
//add replace on fileObj.filePath.replace("/wordpress/wp-admin/","") to workaround on windows server
	...
	test=file;
	var params = 'myajax=1&filename='+ yass_upload_dir+"/"+file.name + '&task=yass_resize_image_and_add&album_id='+jQuery('#multi_album_id').val();
...
function on_queue_completed(file)
{
	...
	yass_refresh_images_table(yass_current_album_id);
...
function yass_refresh_albums_table()
{
	...
	//sleep(2000);
	jQuery.post('index.php', 'task=yass_get_albums_table', function(res)
	{
		yass_set_data_tables();
		if (res.status == 'ok') 
		{
...
function yass_refresh_images_table(album_id)
{
	...
	jQuery.post('index.php', 'task=yass_get_albums_images_table&album_id='+album_id, function(res)
	{
		yass_set_data_tables();
		if (res.status == 'ok') 
		{
...
function re_order()
{
	...
	if( task == 'yass_reorder_album' )
		jQuery('#albums_table tbody').html('<tr><td colspan="7" align="center"><img src="'+loading.src+'" alt="" /><br/>Please wait, refreshing albums</td></tr>');
	if( task == 'yass_reorder_image' )
	...
	jQuery.get('index.php?' + params, function(res)
	{
		if( task == 'yass_reorder_image' )
		{
			yass_refresh_images_table(album_id);
		}
		if( task == 'yass_reorder_album' )
			yass_refresh_albums_table();
...
var tpager = null;
var ipager = null;
function yass_set_data_tables()
{
	//add event to bulk actions
	jQuery('.bulk_form').live('submit', yass_submit_bulk);
...
function yass_submit_bulk()
{
	var task = this.task.value;
	var album_id = null;
	if( task == 'yass_bulk_delete_albums' || task == 'yass_bulk_enable_albums' || task == 'yass_bulk_disable_albums')
	{
	...
	else if( task == 'yass_bulk_delete_images' || task == 'yass_bulk_enable_images' || task == 'yass_bulk_disable_images' )
	{
...
{
	//preload images
	loading.src = yass_url + '/images/loading.gif';
...
/*
		var ops = {
					'uploader'		: yass_url + '/js/uploadify-2.1.4/uploadify.swf',
					'script'		: yass_url + '/js/uploadify-2.1.4/uploadify.php',
					'cancelImg' 	: yass_url + '/js/uploadify-2.1.4/cancel.png',
				    'folder'    	: yass_upload_dir,
				    'auto'			: true,
					'multi'			: true,
					'fileExt'        : '*.jpg;*.gif;*.png',
...
*/
		var swfu;
			...
			var settings = {
				flash_url : yass_url+"/js/swfupload/js/swfupload.swf",
				upload_url: yass_url+"/js/swfupload/js/upload.php",
				post_params: {"PHPSESSID" : "<?php echo session_id(); ?>","folder":yass_upload_dir, "secw":yass_secw, nopar: "n" + new Date().getTime()},
				...
				// Button settings
				button_image_url: yass_url+"/js/swfupload/js/TestImageNoText_65x29.png",
				button_width: "130",
				button_height: "29",
	...
	jQuery('.yass_delete_album').live('click', yass_delete_album);
	...
	jQuery('table#album_images .enable, table#albums_table .enable').live('click', yass_enable);
	jQuery('table#album_images .disable, table#albums_table .disable').live('click', yass_disable);
